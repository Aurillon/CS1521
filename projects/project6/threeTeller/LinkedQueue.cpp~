...
LinkedQueue<ItemType>::LinkedQueue(const LinkedQueue<ItemType>&
                                   aQueue) {
   ...
   if (aQueue.frontPtr) {
      ...
      try {
         frontPtr =
            std::make_shared<Node<ItemType>>(aQueuePtr->getItem() );
      ...
      aQueuePtr.timeLeft = aQueue.getLength();
   }
}
...
bool LinkedQueue<ItemType>::enqueue(const ItemType& newEntry) {
   ...
   if(newEntry.getType() == EventType::ARRIVAL) {
      timeLeft += newEntry.getDuration();
   }
...
bool LinkedQueue<ItemType>::dequeue() {
   ...
   if (ableToDequeue) {
      if (frontPtr == backPtr) {
         ...
         timeLeft -= peekFront().getDuration();
